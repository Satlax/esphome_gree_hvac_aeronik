...
// static const uint8_t SWING = 12; // Swing теперь управляется через Байт 13, а не 12
...
  uint8_t byte13 = data[13];
  ...
  // DISPLAY (Байт 10) - 0x0A/0x0E (ON) vs 0x08/0x06 (OFF)
  // Используем проверку бита 0x02, который включает/выключает дисплей
...
  
  ...
  if (new_mode == AC_MODE_COOL) {
    ...
    byte10 = this->display_state_ ? DISPLAY_ON_MASK : DISPLAY_OFF_MASK;
    this->turbo_state_ = false; // Turbo недоступен в этих режимах
  }
...
// --- Функции для управления переключателями ---
...
// ... (send_data_, dump_message_, get_checksum_ остаются без изменений) ...
